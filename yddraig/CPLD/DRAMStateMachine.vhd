...
-- DRAM controller.
--
-- Changed for 20 MHz CPU operation:
--
--  * Clocked from clk_i (40 MHz global net) gated by en_i instead of from the
--    falling edge of the divided 20 MHz product-term clock.  en_i is asserted
--    on exactly the clk_i edges where the old 20 MHz clock fell, so every
--    RAS/CAS/MUX/WE transition happens at the same absolute time as before and
--    all DRAM AC timing is unchanged.  What goes away is the product-term
--    clock's delay and its skew between function blocks.
--
--  * The refresh counter is 10 bits instead of 20.  It only ever counts to
--    312, so the upper ten bits were ten wasted macrocells -- and ten more
--    macrocells burning current.
--
-- Note that this block is still in the DTACK path for every bus cycle, not
-- just DRAM ones, because the top level ANDs the two DTACK sources together.

entity DRAMStateMachine is
    Port (
        clk_i       : in  std_logic;    -- 40 MHz global clock
        en_i        : in  std_logic;    -- asserted on the old 20 MHz falling edges
...
architecture Behavioral of DRAMStateMachine is
    ...
    signal s_refcount       : unsigned(9 downto 0);         -- The refresh counter
    ...
    -- The refresh counter is clocked once per en_i, i.e. every 50 ns, so
    -- 312 counts is the 15.6 us refresh request interval.
    constant refresh_period : natural := 312;

    -- Per-signal PWR_MODE = "LOW" was tried here to hold the refresh logic in
    -- low power mode while the rest of the design was fitted for speed.  The
    -- attribute works, but it is a bad trade: these twelve macrocells then
    -- become the critical path (23.5 ns against a 25 ns clock) for the sake of
    -- a few mA.  Use the global Power Mode setting instead.
...
-- The second process is a small synchronous process that synchronizes the state transitions and control signal outputs to the system clock.
    ...
    begin
        if (reset_i = '0') then
            ...
            if en_i = '1' then                          -- state machine advances at the old 20 MHz rate
    ...
    s_term_count <= '1' when s_refcount = to_unsigned(refresh_period, s_refcount'length) else '0';

    rreq: process (clk_i, reset_i)
    begin
        if (reset_i = '0') then
            s_refreq <= '0';
        elsif rising_edge(clk_i) then
